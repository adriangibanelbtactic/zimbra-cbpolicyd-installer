...
function create_cbpolicyd_db_and_user () {
  if [[ "$IS_SINGLE" == "TRUE" ]] ; then
    CBPOLICYD_DB_USER_HOST='localhost'
  else
    CBPOLICYD_DB_USER_HOST='%'
  fi

  cat <<EOF > "${CBPOLICYD_DBCREATE_TMP_SQL}"
DROP USER IF EXISTS '${CBPOLICYD_DATABASE_USER}'@'${CBPOLICYD_DB_USER_HOST}';
DROP DATABASE IF EXISTS ${CBPOLICYD_DATABASE};
CREATE DATABASE ${CBPOLICYD_DATABASE} CHARACTER SET 'UTF8'; 
CREATE USER '${CBPOLICYD_DATABASE_USER}'@'${CBPOLICYD_DB_USER_HOST}' IDENTIFIED BY '${CBPOLICYD_PWD}';
GRANT ALL PRIVILEGES ON ${CBPOLICYD_DATABASE} . * TO '${CBPOLICYD_DATABASE_USER}'@'${CBPOLICYD_DB_USER_HOST}' WITH GRANT OPTION;
FLUSH PRIVILEGES ; 
EOF
